...
contains
    ...
    subroutine solve_momentum(assm,rhoi,ui,dt,ap)!(几何网格水力学)（p）（初始条件）（边界条件）（瞬态计算选项）
        ...
        !local
        integer M
        real,allocatable::A(:,:),b(:),pguess(:)
        M=assm%mesh%ny-1
        allocate(A(1:M,1:M),b(1:M),pguess(M+1))
        pguess=assm%thermal%pressure
        call cal_momentumA(assm,pguess,rhoi,ui,dt,A,b,ap)
    ...
    subroutine cal_momentumA(assm,pguess,rhoi,ui,dt,A,b,ap)
     implicit none
     class(sys_assembly),intent(in out)::assm
     real,intent(in)::pguess(:)
      ...
      !local
       ...
       do i=1,N,1
         if(i==1)then
             ...
             ap(i)=api(i)+f/(2*De)*1.50*dx*ulast(i)*(2*RHO(i)+RHO(i+1))/3.0+RHO(1)*uin!replace rho(0)*uin with rho(1)*uin
             bs(i)=pin-pguess(i+1)-(2*RHO(i)+RHO(i+1))/3.0*9.8*1.5*dx+RHO(1)*uin*uin  !replace rho(0)*uin with rho(1)*uin           
...
subroutine solve_pressureCorrection(assm,ap,rhoi,dt,pmodify)
    implicit none
    class(sys_assembly),intent(in out)::assm
    real,intent(in)::ap(:)
    real,intent(in)::rhoi(:)
    ...
    call cal_pmodifyA(assm,ap,rhoi,dt,A,b)
    call solve_pmodifyA(A,b,pmodify)
end subroutine solve_pressureCorrection
    
subroutine cal_pmodifyA(assm,ap,rhoi,dt,A,b)
 implicit none
 class(sys_assembly),intent(in out)::assm
 real,intent(in)::ap(:)
 real,intent(in)::rhoi(:)
    ...
    Do i=1,M-1,1
        Do j=1,N,1
         if (j==1)then!轴对称边界的控制体
          ...
          as(i,j)=0.0
          an(i,j)=0.0
          api(i,j)=RHO(i,j)*SHC(i,j)*assm%mesh%r(i,j)*Df/dt
         ...
         elseif (j==Nf+1)then!f-g边界右侧控制体
          ...
          as(i,j)=0.0
          an(i,j)=0.0
          api(i,j)=RHO(i,j)*SHC(i,j)*assm%mesh%r(i,j)*Dg/dt
          ap(i,j)=aw(i,j)+ae(i,j)+api(i,j)
         ...
         elseif(j==Nf+Ng+1)then!g-c边界右侧控制体
          ...
          as(i,j)=0.0
          an(i,j)=0.0
          api(i,j)=RHO(i,j)*SHC(i,j)*assm%mesh%r(i,j)*Ds/dt
          ap(i,j)=aw(i,j)+ae(i,j)+api(i,j)
         ...
         elseif(j==Nf+Ng+Ns+1)then!fluid控制体
          ...
          if(i==1)then!流体入口的控制体
           ...
           bs(i,j)=Dy/SHC(i,j)*q(i,j)+RHO(i-1,j)*uin*assm%thermal%Temperature(i-1,j)          
          else!流体内部以及出口控制体
           aw(i,j)=Dy/SHC(i,j)*2.0*3.14*Xt/Area*1.0/(1.0/assm%property%htc(i)+Ds/(2*CTC(i,j-1)))
           ae(i,j)=0.0
           as(i,j)=0.0
